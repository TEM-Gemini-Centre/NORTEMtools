...
def preprocess(filename: Union[str, MyPath],
            lazy: bool = True, 
...
def main():
    ...
    parser.add_argument('filename', 
                        type=MyPath, 
                        help='Path to a 4D-STEM dataset to preprocess')
    parser.add_argument('-x', '--cx', 
                        dest='cx', 
                        default=128, 
                        type=int, 
                        help='Center X position of COM mask')
    parser.add_argument('-y', '--cy', 
                        dest='cy', 
                        default=128, 
                        type=int, 
    ...
    parser.add_argument('--estimate_linear_shift', 
    ...
    if calibrations is None:
        ...
        calibrations = {'x': calibrations[0],
                        'y': calibrations[1],
                        'kx': calibrations[2],
                        'ky': calibrations[3]
                        }
    
    logger.debug(f'I will save preprocessed signals in these formats: {arguments.formats}')

    signal = preprocess(arguments.filename, lazy=arguments.lazy, com_mask=(arguments.cx, arguments.cy, arguments.r), estimate_linear_shift=arguments.estimate_linear_shift, width=arguments.width, calibrations=calibrations, nav_chunks=arguments.chunks[0], sig_chunks=arguments.chunks[1])

    # Save the signal
    for f in arguments.formats:
        preprocessed_filename = arguments.filename.with_name(f'{arguments.filename.stem}_preprocessed{f}')
        logger.info(f'Saving preprocessed data to "{preprocessed_filename.absolute()}"')
        chunks = (arguments.chunks[0],)*signal.axes_manager.navigation_dimension + (arguments.chunks[1],)*signal.axes_manager.signal_dimension
        try:
            if f == '.zspy':
                ...
                signal.save(preprocessed_filename, overwrite=arguments.overwrite, chunks=chunks)
            else:
